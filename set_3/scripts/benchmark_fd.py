...
def parse_args():
    ...
    solve.add_argument("--P", type=int, nargs="+", default=[1, 2, 3], choices=[1, 2, 3], help="Pressure solver order(s) (1, 2, or 3)")
    ...
    solve.add_argument("--u0", type=float, default=1.0, help="Initial velocity for benchmarking (default: 1.0)")
    return parser.parse_args()

if __name__ == "__main__":
    args = parse_args()
    print(f"Running benchmarks with parameters: dx={args.dx}, dy={args.dy}, dt={args.dt}, rho={args.rho}, nu={args.nu}")
    print(f"Total simulation time: {args.time} seconds, upwind order: {args.order}, velocity initialization: {args.u0}")
    print(f"Pressure solver parameters: omega={args.omega}, p_threshold={args.p_threshold}, p_max_iters={args.p_max_iters}")
    if 1 in args.P:
        print("Benchmarking P1 solver...")
        fd = FD(dx=args.dx, dy=args.dy, dt=args.dt, rho=args.rho, nu=args.nu, omega=args.omega, u0=args.u0)
        filename = f"set_3/results/iterations_P1_order_{args.order}_u{args.u0}.png"
        fd.benchmark_P1(time=args.time, order=args.order, show=False, save=True, filename=filename,
                        p_threshold=args.p_threshold, p_max_iters=args.p_max_iters)
        filename = f"set_3/results/fields_P1_order_{args.order}_u{args.u0}.png"
        fd.plot(show=False, save=True, filename=filename)
        print("")
    if 2 in args.P:
        print("Benchmarking P2 solver...")
        fd = FD(dx=args.dx, dy=args.dy, dt=args.dt, rho=args.rho, nu=args.nu, omega=args.omega, u0=args.u0)
        filename = f"set_3/results/iterations_P2_order_{args.order}_u{args.u0}.png"
        fd.benchmark_P2(time=args.time, order=args.order, show=False, save=True, filename=filename,
                        p_threshold=args.p_threshold, p_max_iters=args.p_max_iters)
        filename = f"set_3/results/fields_P2_order_{args.order}_u{args.u0}.png"
        fd.plot(show=False, save=True, filename=filename)
        print("")
    if 3 in args.P:
        print("Benchmarking P3 solver...")
        fd = FD(dx=args.dx, dy=args.dy, dt=args.dt, rho=args.rho, nu=args.nu, omega=args.omega, u0=args.u0)
        filename = f"set_3/results/iterations_P3_order_{args.order}_u{args.u0}.png"
        fd.benchmark_P3(time=args.time, order=args.order, show=False, save=True, filename=filename,
                        p_threshold=args.p_threshold, p_max_iters=args.p_max_iters)
        filename = f"set_3/results/fields_P3_order_{args.order}_u{args.u0}.png"
        fd.plot(show=False, save=True, filename=filename)