...
class FD:
    def __init__(self, dx: float = .05, dy: float = .05, dt: float = .001, rho: float = 1.0, nu: float = .1):
        ...
        self.cylinder_mask[0, :] = True   # inlet
        self.cylinder_mask[:, 0] = True   # bottom boundary
        self.cylinder_mask[:, -1] = True  # top boundary
        ...
        self.x_neighbors = convolve2d(~self.cylinder_mask, x_kernel, mode='same', boundary='fill', fillvalue=0)
        self.y_neighbors = convolve2d(~self.cylinder_mask, y_kernel, mode='same', boundary='fill', fillvalue=0)
    ...
    def run(self, time: float, p_threshold: float = 1e-4, p_max_iters: int = 1000):
        ...
        for _ in range(n_iters):
            ...
            self._velocity(self.u, self.v, u_next, v_next, self.p, self.cylinder_mask,
